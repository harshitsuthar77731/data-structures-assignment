...
{ if(t==NULL)
 ...
 return 1;
else return(count_leaf(t->left)+count_leaf(t->right));
 } 
...
count_NONleaf(t->right);
 ...
 { 
Node *root=create();
 Node *t=root; printf("Root child is %d\n ",t->data);
 traverse(t); printf(" is the order traversal \n\n" ); 
 int p=count_leaf(t); int y=count_NONleaf(t); printf("Number of Leaf node=%d\nNumber of Nonleaf=%d",p,y);
 return 0;
 }